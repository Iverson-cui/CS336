...
class Tokenizer:
    ...
    def encode(self, text: str) -> list[int]:
        ...
        for segment in text_segments:
            ...
            # If this segment is a special token
            ...
            word_matches = re.finditer(PAT, segment)
            for match in word_matches:
                word_text = match.group()
                # Convert word to list of token IDs (initially individual bytes)
                token_ids = [
                ...
                tokenid_list.extend(token_ids)

        return tokenid_list

    def _apply_bpe_merges(self, token_ids: list[int]) -> list[int]:
        """
        Apply BPE merges to a list of token IDs in the correct order.
        """
        if len(token_ids) <= 1:
            return token_ids
        ...
        for merge_pair in self.merges:
            # Convert merge pair to token IDs
            left_bytes, right_bytes = merge_pair
            if (
                left_bytes not in self.bytes_to_id
                or right_bytes not in self.bytes_to_id
            ):
                continue
            ...
            if merged_bytes not in self.bytes_to_id:
                continue
            merged_id = self.bytes_to_id[merged_bytes]
...
if __name__ == "__main__":
    ...
    def find_key_position(target_key, dictionary=vocab):
        for index, key in enumerate(dictionary.keys()):
            if key == target_key:
                return index
        return -1  # Not found

    print(find_key_position(b" t"))
